...
class RegisterValidation(Form):
    ...
    email = EmailField('email', [validators.Length(min = 3, max = 50)])
    role = StringField('role', [validators.Length(min = 1, max = 15)])
    contact = StringField('email', [validators.Length(min = 5, message = "Invalid number. Number length should be 10.")])
